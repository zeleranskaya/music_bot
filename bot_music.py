...
hi = ['что ты хочешь послушать сегодня?']
...
other = ['могу предложить что-то еще']
...
def do_other(update, context):
    keyboard = [
        ['настроение', 'жанр'],
    ]

    update.message.reply_text(randomizer.choice(other),
                              reply_markup=ReplyKeyboardMarkup(
                                  keyboard, one_time_keyboard=True, resize_keyboard=True))
